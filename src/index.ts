  ...
  const drafts = [...groups].map(([key, group]) => {
    ...
    return {
      ...
      body: [
        "Dzień dobry",
        "",
        `Zgłaszam nieprawidłowo zaparkowaną hulajnogę ${op.name}, która utrudnia ruch pieszych i/lub stwarza zagrożenie.`,
        "",
        `Data zgłoszenia: ${whenText}`,
        `Numer hulajnogi: ${group[0].id}`,
        `Adres: ${address} (lokalizacja na mapie: https://www.google.com/maps?q=${lat.toFixed(6)},${lng.toFixed(6)})`,
        "",
        "W załączeniu przesyłam zdjęcia hulajnogi pokazujące jak i gdzie została pozostawiona.",
        "",
